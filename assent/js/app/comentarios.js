...
function Comentar() {
    ...
    var hora_coment = $('#hora_coment').val();
    var date_coment = $('#date_coment').val();
    var Link_Online_Trabajo = $("#Link_Online_Trabajo").val();
    if (ImgDelComentario.trim() !== '') {
        $.ajax({
            url: Link_Online_Trabajo,
            method: 'POST',
            data: {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            response.forEach(function(Comentario) {
                ...
                const filtrados = document.createElement('div');
                filtrados.textContent = "Comentado El: "
                filtrados.className = 'filtrados_com';

                var hora_coment = document.createElement('span');
                hora_coment.textContent = Comentario.hora_coment;
                hora_coment.className = 'hora_coment';

                var date_coment = document.createElement('span');
                date_coment.textContent = Comentario.date_coment;
                date_coment.className = 'date_coment';
                filtrados.append(date_coment);
                filtrados.append(hora_coment);
                superr.append(filtrados);
                ...
                const NombreDelComentario = document.createElement('div');
                NombreDelComentario.innerHTML = '<input type="text" value="' + Comentario.NombreDelComentario + '" disabled> dice:';
                //NombreDelComentario.value = Comentario.NombreDelComentario;
                //NombreDelComentario.disabled = true; // Aquí se deshabilita el input
                ...
                if (Comentario.urlcomente) {
                    if (Comentario.urlcomente.match(/\.(jpeg|jpg|gif|png)$/) != null) {
                        const imagen = document.createElement('img');
                        imagen.src = Comentario.urlcomente;
                        imagen.className = 'comentarios-imagen-user-iframe';
                        bajo.append(imagen);
                    } else {
                        const iframecoment = document.createElement('iframe');
                        iframecoment.src = Comentario.urlcomente;
                        iframecoment.className = 'comentarios-iframe-user-iframe';
