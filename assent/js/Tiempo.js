...
var Hora_comentar = document.getElementById("hora_coment")
var Fecha_coment = document.getElementById("date_coment")
...
async function searchByCoordinates(lat, lon) {
    try {
        ...
        Fecha_coment.value = (new Date()).toLocaleDateString();

        const timezoneOffset = data.timezone;
        const localTime = new Date((new Date().getTime() + timezoneOffset + 8)).toLocaleTimeString();
        Hora_comentar.value = new Date((new Date().getTime() + timezoneOffset + 8)).toLocaleTimeString();
